...
int build_vendor(const char* cc, const char* bindir, Cmd* cmd, File_Paths* dirs, File_Paths* c_sources, File_Paths* objs, String_Builder* stb, File_Paths* pathb){
    const char* src_dir = "vendor";
    size_t src_prefix_len = strlen(src_dir)+1;
    if(!walk_directory(dirs, c_sources, src_dir)) return 1;
    for(size_t i = 0; i < dirs->count; ++i) {
        nob_minimal_log_level = NOB_WARNING;
        if(!mkdir_if_not_exists(temp_sprintf("%s/%s", bindir, dirs->items[i] + src_prefix_len))) return 1;
        nob_minimal_log_level = NOB_INFO;
    }
    for(size_t i = 0; i < c_sources->count; ++i) {
        const char* src = c_sources->items[i];
        const char* out = temp_sprintf("%s/%.*s.o", bindir, (int)(strlen(src + src_prefix_len)-2), src + src_prefix_len);
        da_append(objs, out);
...
int main(int argc, char** argv) {
    ...
    int e = build_vendor(cc, bindir, &cmd, &dirs, &c_sources, &objs, &stb, &pathb);
    if(e != 0) return e;
    ...
    const char* exe = temp_sprintf("%s/fed" EXE_SUFFIX, bindir);
